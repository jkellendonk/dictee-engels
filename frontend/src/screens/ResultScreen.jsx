    ...
    <>
      ...
      <div className="panel">
        <div className="result-hero">
          ...
          <div className="result-title">Pakket "{pack.name}" voltooid!</div>
          <div className="result-sub">{praise}</div>
        </div>

        {isRecord && <div className="record-banner">🏆 Nieuw persoonlijk record voor dit pakket!</div>}

        <div className="metric-grid">
          <div className="metric">
            <div className="mval">{result.accuracy}%</div>
            <div className="mlabel">In één keer goed</div>
          </div>
          <div className="metric">
          ...
          <div className="metric">
            <div className="mval">{result.wpm}</div>
            <div className="mlabel">Woorden / minuut</div>
          </div>
          <div className="metric">
          ...
          {top5.map((row, i) => (
            <div className="board-row" key={row.id}>
              ...
              <div className="board-name">
                {row.playerName} <span style={{ color: '#B8C7BF', fontWeight: 600 }}>&middot; {row.pack.name}</span>
              </div>
              <div className="board-stat">{row.accuracy}% &middot; {fmtTime(row.timeSeconds)}</div>
            </div>
          ))}
        </div>

        <div className="result-actions">
          <button className="ghost-btn" onClick={onChangePack}>Ander pakket</button>
          <button className="start-btn" onClick={onPlayAgain}>Nog een keer ➜</button>
        </div>
